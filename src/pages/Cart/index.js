...
class Cart extends Component {
  ...
  componentDidUpdate(_, prevState) {
    if (prevState.ProdutsCart !== this.state.ProdutsCart) {
      localStorage.setItem(
        'productsCompra',
        JSON.stringify(this.state.ProdutsCart),
      );
    }
  }
  ...
  handleDeleteItem(idProduct) {
    this.setState({
      ProdutsCart: this.state.ProdutsCart.filter(
            ...
            {ProdutsCart.map((product) => {
              return (
                <ListItem
                  ...
                  price={product.price}
                  installmentsQuantity={product.installments.map(
                    (item) => item.quantity,
                  )}
                  installmentsValue={product.installments.map(
                    (item) => item.value,
                  )}
